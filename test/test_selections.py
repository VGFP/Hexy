...
def test_get_ring_with_positive_radius():
    r = 3
    expected = (
        _get_linear_combinations(hx.W, hx.NW, r)
    ...
    # assert False
    _assert_np_array_equal(hx.get_ring(np.array([0, 0, 0]), r), expected)


def test_get_ring_with_zero_radius():
    _assert_np_array_equal(hx.get_ring(np.array([0, 0, 0]), 0), [[0, 0, 0]])
...
def test_get_spiral():
    ...
    actual_array = hx.get_spiral(np.array([0, 0, 0]), start_radius, end_radius)
...
def test_get_disk():
    ...
    actual_array = hx.get_disk(np.array([0, 0, 0]), radius)
    expected_array = np.array(expected)
    _assert_np_array_equal(actual_array, expected_array)
...
def test_get_disk_not_at_origin():
    ...
    for r in range(radius + 1):
        expected += (
            _get_linear_combinations(hx.W, hx.NW, r, center)
